...
fnames = glob.glob('QArgon?-50um-?--???.fits')
focus = [int(f[16:19]) for f in fnames]
nfirst=5
threshold=500

fnames = glob.glob('../Feb27/Qe*fits')
...
#Now lets run through the images
...
for j, fn in enumerate(fnames):
    ...
    for i, xyix in enumerate(peaks):
        ...
        if (peakparams[j,i,4]<1.5/2.35) and (i != 2):
            print(focus[j])
            print(xyix)
            print(p)
            plt.clf()
